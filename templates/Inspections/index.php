          ...
          <div class="card">
            ...
            <div class="card-header d-flex justify-content-between align-items-center">
              <h3 class="card-title"><?= __('Liste des relances') ?></h3>
               <div class="ml-auto">
                    <?= $this->Html->link(__('Nouvelle visite'), ['action' => 'add'], [
              ...
              <table id="example1" class="table table-bordered table-hover table-sm">
                <thead>
                  <tr>
                    ...
                    <th><?= __('genre') ?></th>
                    <th><?= __('Statut') ?></th>
                    <th><?= __('Prochaine visite') ?></th>
                    <th class="text-center"><?= __('Actions') ?></th>
                  </tr>
                </thead>
                <tbody>
                <?php $count = 1; foreach ($inspections as $inspection): ?>
                <tr>
                    <td><?= $count++ ?></td>
                     <td><?= $inspection->vehicle_id ?></td>
                    <td><?= $inspection->customer_id ?></td>
                    <td><?= h($inspection->gender_id) ?></td>
                    <td><?= h($inspection->status) ?></td>
                    <td><?= h($inspection->end_date) ?></td>
                    <td class="actions">
                          <?= $this->Html->link(__('<i class="fas fa-eye" style="color:#000;"></i>'), ['action' => 'view', $inspection->id], ['escape' => false, 'title'=>'Consulter']) ?>
                          <?= $this->Html->link(__('<i class="fas fa-edit" style="color:#000;"></i>'), ['action' => 'edit', $inspection->id], ['escape' => false,'title'=>'Modifier']) ?>
                         <?= $this->Form->postLink(
