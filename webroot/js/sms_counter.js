  ...
  window.SmsCounter = SmsCounter = (function() {
    ...
    SmsCounter.count = function(text, dest) {
      var count, encoding, length, messages, per_message, remaining, destCount, sms_count; 
      encoding = this.detectEncoding(text);
      length = text.length;
      destCount = this.countReceivers(dest);
      sms_count = 0;
      ...
      return count = {
        ...
        remaining: remaining,
        messages: messages,
        sms_count: messages * destCount
      };
    };

    SmsCounter.countReceivers = function(text) {
      var count=0;
      ...
      if (text.length <= 7 ) {
        ...
        let destArray = text.split(';');
        const destFiltered = [];

        for (var i = destArray.length - 1; i >= 0; i--) {
          if(destArray[i].length >=8) {
            // vérifie si le numéro existe déjà ou pas
            if (!destFiltered.includes(destArray[i])){
              destFiltered[count] = destArray[i];
              count++;  
            }
          }
        }
  ...
  if (typeof jQuery !== "undefined" && jQuery !== null) {
    $ = jQuery;
    $.fn.countSms = function(target, receivers) {
      var count_sms, input, dest;
      input = this;
      target = $(target);
      dest = $(receivers);
      ...
      count_sms = function() {
        ...
        count = SmsCounter.count(input.val(), dest.val());
        _results = [];
        for (k in count) {
          v = count[k];
